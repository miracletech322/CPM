...
@if ($paginator->hasPages())
    <nav>
        <ul class="pagination">
            {{-- Previous Page Link --}}
            @if ($paginator->onFirstPage())
                <li class="disabled btn border-2 ml-1  " aria-disabled="true" aria-label="@lang('pagination.previous')">
                    <span aria-hidden="true">&lsaquo;</span>
                </li>
            @else
                <li class='btn border-2 ml-1  '>
            ...
            @foreach ($elements as $element)
                ...
                @if (is_string($element))
                    <li class="disabled btn border-2 ml-1  " aria-disabled="true"><span>{{ $element }}</span></li>
                @endif
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <li class="active btn border-2 ml-1  " aria-current="page"><span>{{ $page }}</span></li>
                        @else
                            <li class="btn border-2 ml-1"><a href="{{ $url }}">{{ $page }}</a></li>
                        @endif
                    @endforeach
                @endif
            @endforeach
            ...
            @else
                <li class="disabled btn border-2 ml-1  " aria-disabled="true" aria-label="@lang('pagination.next')">
